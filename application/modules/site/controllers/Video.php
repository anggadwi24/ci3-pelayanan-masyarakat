...
class Video extends MX_Controller {
    ...
    public function index() {
            ...
            $data['js'] = base_url('template/public/js/ajax/gallery/ajax-video.js');
    ...
    function data($rowno){
        if($this->input->method() == 'post'){
            ...
            $allcount = $this->model_app->join_where_order2('gallery','gallery_video','gal_id','vid_gal_id',array('gal_visible'=>'y'),'gal_date','DESC')->num_rows();
            $data = $this->model_app->join_where_order_limit('gallery','gallery_video','gal_id','vid_gal_id',array('gal_visible'=>'y'),'gal_date','DESC',$rowperpage,$rowno);
            $config['base_url'] = base_url().'site/video/index/';
            ...
            if($data->num_rows() > 0){  
                ...
                foreach($data->result_array() as $row){
                    ...
                    if($row['gal_thumbnail'] != NULL){
                        ...
                        $img = base_url('upload/docs/video.jpg');
                    }
                        $play = ' <div class="binduz-er-play">
                        <a class="binduz-er-video-popup" href="'.base_url('gallery/'.$row['gal_seo']).'"><i class="fas fa-play"></i></a>
                    </div>';
